...
export function generateWave(params: WaveParams, sampleCount = 128): number[] {
  ...
  for (let i = 0; i < sampleCount; i += 1) {
    ...
    const symmetry = Math.max(0.5, Math.min(1.5, params.symmetry || 1));
    const skewedT = Math.pow(t, symmetry);
    const phase = skewedT * Math.PI * 2 * params.frequency + params.phase;
    ...
    const complexWave = base + (harmonic2 + harmonic3) * params.harmonics;
    
    // Decay: higher decay means wave shrinks faster towards the right
    const damped = complexWave * (1 - t * params.decay);
